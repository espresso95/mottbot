...
export class WorktreeManager {
  ...
  mergeBranch(params: { worktreePath: string; branchName: string }): MergeResult {
    try {
      ...
      const output = [
        typeof maybe.stdout === "string" ? maybe.stdout : Buffer.isBuffer(maybe.stdout) ? maybe.stdout.toString("utf8") : undefined,
        typeof maybe.stderr === "string" ? maybe.stderr : Buffer.isBuffer(maybe.stderr) ? maybe.stderr.toString("utf8") : undefined,
  ...
  cleanupSubtask(params: { repoRoot: string; worktreePath?: string; branchName?: string; deleteBranch?: boolean }): void {
    const repoRoot = this.resolveApprovedRepoRoot(params.repoRoot);
