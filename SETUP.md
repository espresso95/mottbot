...
- Agent entries can set `profileId`, `modelRef`, `fastMode`, `systemPrompt`, `toolNames`, and `toolPolicies`. `toolNames` narrows the tools exposed to that agent; `toolPolicies` narrows the global tool policy for specific tools.
